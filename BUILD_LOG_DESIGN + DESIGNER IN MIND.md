...
- **Two entry types, one shell.** Every entry shares the shell (ghost numeral, entry-id, title, thesis) — but a `kind` field (`'build' | 'design'`) determines what fills it:
  - `build`: status badge with colored dot (`live`/`in-progress`), small schematic graphic, stack tags, "one real decision" line, GitHub/live links.
  - `design`: plain type-label badge (no dot — a design piece isn't "shipping" the same way a code project is), full-width image as the entry's hero (not a supporting diagram), tool/medium tags, "the brief" line instead of a decision line, an optional case-study link.
- **Order is chronological, not grouped by kind.** Design and build entries interleave in the log by when they actually happened — the mixed order is itself the proof of "designer and developer," not a claim made in the About copy. 3–4 design pieces are planned; their exact position in the sequence depends on when each was made relative to the code projects.
